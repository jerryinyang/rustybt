...
## Status
Partially Complete - Phase 1 & 4 Complete
...
- [ ] Test migration script (AC: 2.1-2.6)
  - [ ] Test dry-run mode (no changes committed)
  - [ ] Test backup creation (SHA256 checksums verified)
  - [ ] Test transactional migration (rollback on error)
  - [ ] Test validation checkpoints (row count comparisons)
  - [ ] Test rollback command (restore from backup)
...
- Fixed migration script to use correct table names (bundle_path vs parquet_path)
- Updated to use INSERT OR IGNORE for idempotent migrations
- Added proper error handling to continue on bundle-specific failures
- Migration script ready for testing (Phase 2)
...
- scripts/migrate_catalog_to_unified.py (modified - fixed table names and error handling)
