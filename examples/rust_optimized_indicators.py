...
def calculate_indicators_python(prices, short_window=20, long_window=50):
    ...
    sma_diff = [s - l for s, l in zip(sma_short, sma_long)]
    ema_diff = [s - l for s, l in zip(ema_short, ema_long)]
...
def benchmark_performance(n_prices=10000, iterations=100):
    ...
    print(f"Python implementation: {python_time:.4f}s ({python_time/iterations*1000:.2f}ms per iteration)")
    ...
    if RUST_AVAILABLE:
        ...
        print(f"Rust implementation:   {rust_time:.4f}s ({rust_time/iterations*1000:.2f}ms per iteration)")
