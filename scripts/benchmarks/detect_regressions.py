...
BASELINES_FILE = Path(__file__).parent.parent.parent / "tests" / "regression" / "performance_baselines.json"
...
def load_baselines() -> Dict[str, Any]:
    ...
    if not BASELINES_FILE.exists():
        ...
        print("Run: pytest tests/regression/test_performance_regression.py::test_create_baselines", file=sys.stderr)
        return {}
    ...
    if failures or warnings:
        ...
        if github_output.exists():
            with open(github_output, 'a') as f:
                f.write(f"regressions_detected=true\n")
                f.write(f"regression_report<<EOF\n{create_github_issue_body(warnings, failures)}\nEOF\n")
