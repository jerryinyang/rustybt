...
def generate_execution_time_chart(history: Dict[str, Any]) -> Optional[str]:
...
def generate_scenario_comparison_chart(history: Dict[str, Any]) -> Optional[str]:
    ...
    for run in runs:
        ...
        if scenario not in latest_by_scenario or timestamp > latest_by_scenario[scenario].get('timestamp', ''):
            latest_by_scenario[scenario] = run
    
    # Sort by execution time
    scenarios = sorted(latest_by_scenario.items(), 
...
def generate_html_dashboard(history: Dict[str, Any]) -> str:
    ...
    runs_table = "\n".join(runs_table_rows) if runs_table_rows else "<tr><td colspan='4'>No benchmark runs recorded</td></tr>"
