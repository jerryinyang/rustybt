...
def generate_ohlcv_data(
    ...
    logger.info(f"Generating {len(timestamps)} bars for {num_assets} assets at {frequency} frequency")
...
def main():
    ...
    parser.add_argument(
        '--assets',
        type=int,
        help='Number of assets (only used with --frequency, not "all")'
    )
    ...
    if args.frequency == 'all':
        ...
        filename = f"{args.frequency.replace('1', '')}{args.frequency[-1]}_{args.assets}_assets.parquet"
