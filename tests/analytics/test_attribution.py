...
def test_alpha_beta_with_known_parameters(synthetic_alpha_beta_data):
    ...
    assert abs(estimated_alpha - true_alpha) < 0.001, \
        f"Alpha estimation error too large: {estimated_alpha} vs {true_alpha}"

    assert abs(estimated_beta - true_beta) < 0.2, \
...
def test_factor_attribution_with_fama_french_factors():
    ...
    for factor, true_loading in true_loadings.items():
        ...
        assert abs(estimated_loading - true_loading) < 0.3, \
...
def test_analyze_attribution_full_workflow(sample_returns, sample_benchmark_returns, sample_factor_returns):
...
@given(
    n_obs=st.integers(min_value=30, max_value=100)
)
