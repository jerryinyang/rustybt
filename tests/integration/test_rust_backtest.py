...
from rustybt.rust_optimizations import RUST_AVAILABLE, rust_ema, rust_sma, rust_pairwise_op
        ...
        for i in range(1, len(results)):
            for j in range(len(results[0])):
                if math.isnan(results[0][j]):
                    ...
                    assert results[0][j] == results[i][j], f"Results should be deterministic at index {j}"
    ...
    def test_rust_python_equivalence_in_strategy(self):
        ...
        for i in range(len(prices)):
            if math.isnan(python_result[i]):
                ...
                assert math.isclose(rust_result[i], python_result[i], rel_tol=1e-9), \
...
class TestRustOptimizationFallback:
    ...
    def test_fallback_produces_correct_results(self):
        ...
        from rustybt.rust_optimizations import rust_sma, rust_ema, rust_array_sum
