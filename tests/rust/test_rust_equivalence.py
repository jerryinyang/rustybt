...
def test_sma_equivalence(values, window):
    ...
    assert arrays_equal(rust_result, python_result), \
...
def test_array_sum_equivalence(values):
    ...
    assert math.isclose(rust_result, python_result, rel_tol=1e-9), \
...
def test_pairwise_op_equivalence(size, op):
    ...
    if op == 'add':
        python_result = [x + y for x, y in zip(a, b)]
    elif op == 'sub':
        python_result = [x - y for x, y in zip(a, b)]
    elif op == 'mul':
        python_result = [x * y for x, y in zip(a, b)]
    ...
    assert arrays_equal(rust_result, python_result, rel_tol=1e-7), \
...
def test_decimal_sma_equivalence_fixed_precision():
    ...
    rust_result = rust_sma(values, window)
    for rust_val, py_val in zip(rust_result, python_result):
...
decimal_strategy = st.builds(
    ...
    st.one_of(
        ...
        st.floats(min_value=-1_000_000.0, max_value=1_000_000.0, allow_nan=False, allow_infinity=False).map(str),
    ).map(str)
)
...
def test_decimal_sma_property_based(values, window):
    ...
    assert decimals_equal(rust_result, python_result, places=8), \
...
def test_decimal_sum_property_based(values):
    ...
    assert decimals_equal([rust_result], [python_result], places=10), \
...
def test_decimal_pairwise_op_property_based(size, op):
    ...
    if op == 'add':
        ...
        ctx = getcontext()
        python_result = [ctx.divide(x, y) for x, y in zip(a, b)]
...
def test_decimal_precision_preservation():
    ...
    try:
        ...
        assert result == Decimal("0.6"), \
            f"Decimal precision lost: expected 0.6, got {result}"
        ...
        assert abs(float_result - 0.3) > 1e-17, \
...
def test_decimal_rounding_modes():
    ...
    from decimal import getcontext, ROUND_HALF_UP, ROUND_DOWN
    ...
    try:
        ...
        assert all(isinstance(x, Decimal) for x in result_half_up if not (isinstance(x, Decimal) and x.is_nan()))
        
        # Test ROUND_DOWN
        getcontext().rounding = ROUND_DOWN
        result_down = rust_sma(values, window)
        assert all(isinstance(x, Decimal) for x in result_down if not (isinstance(x, Decimal) and x.is_nan()))
