...
from rustybt.api import FixedSlippage
from rustybt.assets import Asset, Equity, Future
from rustybt.assets.continuous_futures import ContinuousFuture
from rustybt.assets.synthetic import make_jagged_equity_info, make_simple_equity_info
...
from rustybt.finance.commission import PerShare, PerTrade
from rustybt.finance.controls import AssetDateBounds
...
from rustybt.test_algorithms import (
...
class TestRecord(zf.WithMakeAlgo, zf.ZiplineTestCase):
    ...
    def test_record_incr(self):
        ...
        def handle_data(self, data):
            ...
            rustybt.api.record(name, self.incr, "name2", 2, name3=self.incr)
...
class TestMiscellaneousAPI(zf.WithMakeAlgo, zf.ZiplineTestCase):
    ...
    def test_cancel_policy_outside_init(self):
        code = dedent(
            """
            from rustybt.api import cancel_policy, set_cancel_policy
    ...
    def test_cancel_policy_invalid_param(self):
        code = dedent(
            """
            from rustybt.api import set_cancel_policy
    ...
    def test_zipline_api_resolves_dynamically(self):
        ...
        for method in algo.all_api_methods():
            ...
            with ZiplineAPI(algo):
                assert sentinel is getattr(rustybt.api, name)()

    def test_sid_datetime(self):
        algo_text = dedent(
            """
            from rustybt.api import sid, get_datetime
            import pandas as pd
    ...
    def test_datetime_bad_params(self):
        algo_text = dedent(
            """
            from rustybt.api import get_datetime
...
class TestBeforeTradingStart(zf.WithMakeAlgo, zf.ZiplineTestCase):
    ...
    def test_data_in_bts_minute(self):
        algo_code = dedent(
            """
            from rustybt.api import record, sid
            def initialize(context):
    ...
    def test_portfolio_bts(self):
        algo_code = dedent(
            """
            from rustybt.api import order, sid, record
    ...
    def test_account_bts(self):
        algo_code = dedent(
            """
            from rustybt.api import order, sid, record, set_slippage, slippage
...
class TestAlgoScript(zf.WithMakeAlgo, zf.ZiplineTestCase):
    ...
    def test_incorrectly_set_futures_slippage_model(self):
        code = dedent(
            """
            from rustybt.api import set_slippage, slippage
    ...
    def test_batch_market_order_matches_multiple_manual_orders(self):
        ...
        multi_test_algo = self.make_algo(
            script=dedent(
                ...
                from rustybt.api import sid, order
        ...
        batch_test_algo = self.make_algo(
            script=dedent(
                ...
                from rustybt.api import sid, batch_market_order
    ...
    def test_order_dead_asset(self):
        ...
        self.run_algorithm(
            script=dedent(
                """
                from rustybt.api import order, sid
        ...
        for order_str in ["order_value", "order_percent"]:
            ...
            script_template = dedent(
                """
                from rustybt.api import order_percent, order_value, sid
    ...
    def test_schedule_function_time_rule_positionally_misplaced(self):
        ...
        algocode = dedent(
            """
        from rustybt.api import time_rules, schedule_function
...
class TestCapitalChanges(zf.WithMakeAlgo, zf.ZiplineTestCase):
    ...
    def test_capital_changes_minute_mode_minute_emission(self, change, values):
        ...
        algocode = """
from rustybt.api import set_slippage, set_commission, slippage, commission, \
