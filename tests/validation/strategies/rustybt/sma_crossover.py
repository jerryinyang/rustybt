    ...
    def _execute_signal(
        ...
        if signal == "BUY" and self._position_state == 0:
            ...
            if shares_to_buy > 0:
                ...
                # Log transaction (Layer 4) - simplified since rustybt handles execution
                self.log_transaction(
                    asset=asset_name,
                    quantity=shares_to_buy,
                    price=current_price,
                    commission=1.0,  # Default commission
                    slippage=0.0,
                )
            ...
            # Get current position size for logging
            shares_to_sell = self._get_position_size(context)
